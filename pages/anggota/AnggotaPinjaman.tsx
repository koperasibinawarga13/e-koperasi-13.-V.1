...
const AnggotaPinjaman: React.FC = () => {
    ...
    const InfoItem: React.FC<{ label: string; value: string; className?: string }> = ({ label, value, className = '' }) => (
        <div className={`flex justify-between items-center py-2 border-b border-slate-200 ${className}`}>
    ...
    return (
        <div>
            ...
            <div className="bg-surface rounded-xl border border-slate-200 mb-8 p-6">
                    ...
                    {isLoading ? <p className="text-gray-text">Memuat data sisa pinjaman...</p> : keuangan ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-amber-100 p-4 rounded-lg">
                                <p className="text-sm text-amber-700 font-medium">Sisa Pinjaman Berjangka</p>
                                <p className="text-xl md:text-2xl font-bold text-amber-800">{formatCurrency(keuangan.akhir_pinjaman_berjangka)}</p>
                            </div>
                            <div className="bg-orange-100 p-4 rounded-lg">
                                <p className="text-sm text-orange-700 font-medium">Sisa Pinjaman Khusus</p>
                                <p className="text-xl md:text-2xl font-bold text-orange-800">{formatCurrency(keuangan.akhir_pinjaman_khusus)}</p>
                            </div>
                        </div>
            ...
            <div className="bg-surface rounded-xl border border-slate-200">
                <h2 className="text-lg md:text-xl font-bold text-dark bg-slate-100 p-4 rounded-t-xl -m-0">Simulasi & Pengajuan Kredit</h2>
                 <nav className="flex border-b border-slate-200">
                    <button onClick={() => {setActiveTab('berjangka'); setSubmitMessage({type: '', text: ''})}} className={`py-3 px-6 font-semibold text-sm ${activeTab === 'berjangka' ? 'text-primary border-b-2 border-primary' : 'text-gray-text hover:bg-slate-100'}`}>Pinjaman Berjangka (Dengan Simulasi)</button>
                    <button onClick={() => {setActiveTab('khusus'); setSubmitMessage({type: '', text: ''})}} className={`py-3 px-6 font-semibold text-sm ${activeTab === 'khusus' ? 'text-primary border-b-2 border-primary' : 'text-gray-text hover:bg-slate-100'}`}>Pinjaman Khusus</button>
                 </nav>
                    ...
                    <div className="p-6">
                        <form onSubmit={handleCalculate} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-end">
                            <div>
                                ...
                                <input type="text" inputMode="numeric" id="pokok" value={new Intl.NumberFormat('id-ID').format(pokokPinjaman)} onChange={handleCurrencyChange(setPokokPinjaman)} className="mt-1 block w-full bg-slate-50 border border-slate-300 rounded-md py-2 px-3 text-dark"/>
                            </div>
                             <div>
                                <label htmlFor="jangka" className="block text-sm font-medium text-gray-text">Jangka Waktu (Bulan)</label>
                                <input type="number" id="jangka" value={jangkaWaktu} onChange={e => setJangkaWaktu(Number(e.target.value))} className="mt-1 block w-full bg-slate-50 border border-slate-300 rounded-md py-2 px-3 text-dark" />
                            </div>
                             <div>
                                <label htmlFor="bunga" className="block text-sm font-medium text-gray-text">Bunga per Bulan (%)</label>
                                <input type="number" step="0.1" id="bunga" value={sukuBunga} onChange={e => setSukuBunga(Number(e.target.value))} className="mt-1 block w-full bg-slate-50 border border-slate-300 rounded-md py-2 px-3 text-dark" />
                            </div>
                            <div>
                             ...
                             <div className="md:col-span-2 lg:col-span-4">
                                ...
                                <input type="date" id="tanggal" value={tanggalMulai} onChange={e => setTanggalMulai(e.target.value)} className="mt-1 block w-full md:w-1/4 bg-slate-50 border border-slate-300 rounded-md py-2 px-3 text-dark" />
                            </div>
                        </form>
                        {simulasi && (
                        <div className="mt-8 text-center border-t border-slate-200 pt-6">
                            <button onClick={handleAjukanPinjamanBerjangka} disabled={isSubmitting} className="bg-secondary text-white py-3 px-8 rounded-lg font-bold text-base md:text-lg hover:bg-secondary-dark disabled:bg-slate-400">
                    ...
                    <div className="p-6">
                         <form onSubmit={handleAjukanPinjamanKhusus} className="space-y-4">
                             <div>
                                ...
                                <input type="text" inputMode="numeric" id="pokokKhusus" value={new Intl.NumberFormat('id-ID').format(pokokPinjamanKhusus)} onChange={handleCurrencyChange(setPokokPinjamanKhusus)} className="mt-1 block w-full bg-slate-50 border border-slate-300 rounded-md py-2 px-3 text-dark" required />
                            </div>
                             <div>
                                <label htmlFor="keterangan" className="block text-sm font-medium text-gray-text">Keterangan / Tujuan Pinjaman</label>
                                <textarea id="keterangan" value={keteranganKhusus} onChange={e => setKeteranganKhusus(e.target.value)} rows={3} className="mt-1 block w-full bg-slate-50 border border-slate-300 rounded-md py-2 px-3 text-dark" required></textarea>
                            </div>
                             <div className="text-center pt-2">
                                 <button type="submit" disabled={isSubmitting} className="bg-secondary text-white py-2 px-6 rounded-lg font-semibold hover:bg-secondary-dark disabled:bg-slate-400">
                ...
                {simulasi && activeTab === 'berjangka' && (
                    <div className="mt-8 border-t border-slate-200 pt-6 px-6 pb-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                             <div>
                                <h3 className="font-bold text-base md:text-lg text-dark mb-2">Informasi Pinjaman Anda</h3>
                                <div className="bg-slate-100 p-4 rounded-lg"><InfoItem label="Pokok Pinjaman" value={formatCurrency(simulasi.pokokPinjaman)} /><InfoItem label="Jangka Waktu" value={`${simulasi.jangkaWaktu} Bulan`} /><InfoItem label="Bunga per Bulan" value={`${simulasi.sukuBunga} %`} /><InfoItem label="Tanggal Mulai" value={formatDate(simulasi.tanggalMulai)} /></div>
                             </div>
                             <div>
                                <h3 className="font-bold text-base md:text-lg text-dark mb-2">Informasi Angsuran Anda</h3>
                                <div className="bg-slate-100 p-4 rounded-lg"><InfoItem label="Angsuran Pokok / Bulan" value={formatCurrency(simulasi.angsuranPokokBulan)} /><InfoItem label="Total Bunga" value={formatCurrency(simulasi.totalBunga)} /><InfoItem label="Total yang Dibayarkan" value={formatCurrency(simulasi.totalBayar)} className="font-bold text-base md:text-lg bg-amber-100 text-amber-800 -mx-4 px-4" /><InfoItem label="Tanggal Lunas" value={formatDate(simulasi.tanggalLunas)} /></div>
                             </div>
                        </div>
            ...
            <div className="mt-8 bg-surface p-6 rounded-xl border border-slate-200">
                 ...
                 <div className="overflow-x-auto">
                     {isLoading ? <p className="text-gray-text">Memuat riwayat...</p> : riwayat.length > 0 ? (
                         <table className="w-full text-sm text-left text-gray-text">
                            <thead className="text-xs text-gray-text uppercase bg-slate-100">
                                <tr>
                                         ...
                                         <td className="px-4 py-3 text-center">
                                            {p.status === 'Menunggu Persetujuan' ? (
                                                <button
                                                    ...
                                                    className="text-red-600 hover:text-red-700 text-xs font-semibold disabled:text-slate-400"
                                                >
