...
const RichTextEditor: React.FC<RichTextEditorProps> = ({ value, onChange }) => {
    ...
    const ToolbarButton: React.FC<{
        ...
        <button
            ...
            title={title}
            className="p-2 rounded text-slate-500 hover:bg-slate-200 hover:text-dark focus:outline-none focus:ring-2 focus:ring-primary"
        >
            {children}
        </button>
    );
    ...
    return (
        <div className="bg-slate-100 rounded-md mt-1">
            <div className="flex items-center flex-wrap gap-1 p-2 bg-slate-100 rounded-t-md">
                ...
                {selectedImage && (
                    <>
                        <div className="w-px h-6 bg-slate-300 mx-2"></div>
                        <span className="text-xs font-semibold text-gray-text mr-2">Gambar:</span>
                        ...
                        <div className="w-px h-6 bg-slate-300 mx-2"></div>
                         <button onClick={() => handleImageStyle({ width: '25%' })} className="text-xs px-2 py-1 rounded hover:bg-slate-200 text-gray-text">25%</button>
                         <button onClick={() => handleImageStyle({ width: '50%' })} className="text-xs px-2 py-1 rounded hover:bg-slate-200 text-gray-text">50%</button>
                         <button onClick={() => handleImageStyle({ width: '75%' })} className="text-xs px-2 py-1 rounded hover:bg-slate-200 text-gray-text">75%</button>
                         <button onClick={() => handleImageStyle({ width: '100%' })} className="text-xs px-2 py-1 rounded hover:bg-slate-200 text-gray-text">100%</button>
                    </>
                )}
            </div>
            <div
                ...
                className="prose min-h-[200px] w-full max-w-none p-3 focus:outline-none bg-white text-dark rounded-b-md"
            />
            <input
                type="file"
                ref={fileInputRef}
            ...
            />
             <style>{`
                .prose p { color: #475569; } /* slate-600 */
                .prose h1, .prose h2, .prose h3, .prose strong { color: #1e2937; } /* slate-800 */
                .prose a { color: #0ea5e9; } /* primary */
                .prose ul, .prose ol { color: #475569; }
            `}</style>
        </div>
    );
};
